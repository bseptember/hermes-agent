...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    # Skip password-only providers — auto-SSO /auth/login is an OAuth-only
    # endpoint. Password providers (supports_password=True) render a login
    # form at /login; redirecting to /auth/login on them raises
    # NotImplementedError → 500. They are NOT valid auto-SSO candidates.
